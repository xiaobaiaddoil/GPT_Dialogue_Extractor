...
(function () {
  ...
  function serializeCodeBlock(node) {
    const codeNode =
      node.matches(CODE_CONTENT_SELECTOR) || node.tagName === "PRE"
        ? node
  ...
  function getAssetCardLabel(card) {
    ...
    if (filenameNode instanceof HTMLElement) {
      const filenameLabel =
        filenameNode.getAttribute("aria-label") ||
  ...
  function isDecorativeAssetImage(image, url, alt) {
    ...
    if (
      lowerUrl.includes("drive-thirdparty.googleusercontent.com/32/type/") ||
  ...
  function findTurnElement(turnId, message) {
    ...
    return (
      document.querySelector(`section[data-testid="${escapedTurnId}"]`) ||
