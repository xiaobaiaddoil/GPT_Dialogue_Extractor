...
(function () {
    ...
    function isSupportedConversationPage() {
        return isChatGPTPage() || isGeminiPage() || isDeepSeekPage();
    }
    ...
    function getMessageDomTurnIds(message) {
        ...
        return Array.from(new Set([message.backendNodeId, message.turnNodeId, message.backendMessageId, getMessageBackendId(message), message.turnId]
    ...
    function mergeConversationMessage(previousMessage, nextMessage) {
        ...
        const previousSequence = typeof previousMessage.sequenceIndex === "number" && Number.isFinite(previousMessage.sequenceIndex)
            ? previousMessage.sequenceIndex
            : null;
        const nextSequence = typeof nextMessage.sequenceIndex === "number" && Number.isFinite(nextMessage.sequenceIndex)
            ? nextMessage.sequenceIndex
            : null;
        const backendSequence = previousMessage.sequenceSource === "backend" && previousSequence !== null
            ? previousSequence
            : nextMessage.sequenceSource === "backend" && nextSequence !== null
                ? nextSequence
                : null;
        const turnId = nextMessage.sequenceSource === "backend" && previousMessage.turnId
    ...
    function mergeConversationFragments(previousConversation, visibleConversation) {
        ...
        function appendOrMergeMessage(message) {
            ...
            const textKey = getMessageTextMergeKey(message);
            const existingIndex = (message.id && indexById.has(message.id) ? indexById.get(message.id) : undefined) ??
    ...
    function getBackendConversationNodes(payload) {
        ...
        const currentNodeId = (typeof payload.current_node === "string" && payload.current_node) ||
    ...
    function renderSelectionList(conversation) {
        ...
        conversation.messages.forEach((message, index) => {
            ...
            row.setAttribute("aria-label", `选择${getMessageDisplayName(message)}消息`);
    ...
    function getTurnBodyForRole(turn, role) {
        ...
        if (turn.matches(DEEPSEEK_VIRTUAL_ITEM_SELECTOR) || turn.querySelector(DEEPSEEK_ASSISTANT_BODY_SELECTOR)) {
            const deepSeekBody = resolveDeepSeekMessageBody(turn, role);
            return deepSeekBody instanceof HTMLElement ? deepSeekBody : turn;
        }
        if (role === "user") {
    ...
    function elementHasAnyMessageDomId(element, ids) {
        ...
        return ids.some((id) => element.getAttribute("data-message-id") === id ||
    ...
    function resolveTurnByMessageDomId(message) {
        ...
        const directTurn = resolveTurns().find((turn) => turn instanceof HTMLElement && turnContainsAnyMessageDomId(turn, domIds));
        if (directTurn instanceof HTMLElement) {
            return directTurn;
        }
        const node = Array.from(document.querySelectorAll("[data-message-id], [data-turn-id], [data-turn-id-container]")).find((candidate) => elementHasAnyMessageDomId(candidate, domIds));
    ...
    function resolveTurnByText(message) {
        ...
        return (resolveTurns().find((turn) => turn instanceof HTMLElement && turnLooksLikeMessage(turn, message)) || null);
    }
    function resolveTimelineTurn(message) {
        ...
        if (message.platform === "deepseek") {
            ...
            if (typeof message.turnId === "string" && message.turnId) {
                ...
                const attrTurn = querySelectorSafe(buildAttributeSelector(DEEPSEEK_TURN_ID_ATTR, message.turnId));
                if (attrTurn instanceof HTMLElement) {
                    return attrTurn;
                }
            }
        ...
        if (message.platform === "gemini") {
            ...
            if (typeof message.turnId === "string" && message.turnId) {
                const turn = querySelectorSafe(buildAttributeSelector(GEMINI_TURN_ID_ATTR, message.turnId));
                if (turn instanceof HTMLElement) {
                    return turn;
                }
            }
    ...
    function resolveLatestTimelineMessage(message) {
        ...
        const backendMessageId = getMessageBackendId(message);
        const textKey = getMessageTextMergeKey(message);
        return (latestConversation.messages.find((candidate) => candidate.id === message.id) ||
            (backendMessageId
                ? latestConversation.messages.find((candidate) => candidate.role === message.role && getMessageBackendId(candidate) === backendMessageId)
                : null) ||
    ...
    function resolveTimelineTargetElement(message) {
        ...
        if (latestMessage && latestMessage.platform === "gemini") {
            const body = resolveGeminiMessageBody(turn, latestMessage.role);
            return body instanceof HTMLElement ? body : turn;
        }
        if (latestMessage && latestMessage.platform === "deepseek") {
            const body = resolveDeepSeekMessageBody(turn, latestMessage.role);
            return body instanceof HTMLElement ? body : turn;
        }
        const body = getTurnBodyForRole(turn, latestMessage && latestMessage.role);
        return body instanceof HTMLElement ? body : turn;
    }
    ...
    function refreshActiveTimelineMessage() {
        ...
        const scrollerRect = currentTimelineScroller instanceof HTMLElement
    ...
    async function collectConversationForAction(options = {}) {
        ...
        if (options.forceBackend) {
            return collectConversationSnapshot(true);
        }
        return collectConversationSnapshot(false);
    }
    ...
    function syncLiveConversation() {
        ...
        const nextConversation = canMergeWithDeepSeekHistory(latestConversation)
            ...
            : mergeConversationFragments(latestConversation, visibleConversation);
    ...
    function scheduleTimelineSettledRefreshes() {
        ...
        timelineSettledRefreshTimers = [600, 1400, 2800].map((delayMs) => window.setTimeout(() => {
            requestTimelineRefresh();
    ...
    function ensureToolbar() {
        ...
        const shouldUseFloatingToolbar = !headerActions && (isGeminiPage() || isDeepSeekPage());
    ...
    function ensurePanel() {
        ...
        status.style.color = "#6b7280";
        panel.append(header, description, scope, actions, selectionHeader, selectionSummary, messageList, status);
    ...
    function serializeTable(node) {
        ...
        const serializedRows = rows.map((row) => Array.from(row.querySelectorAll("th, td"))
    ...
    function hasDeepSeekMessageText(body) {
        return Boolean(body instanceof HTMLElement && normalizeWhitespace(readExportText(body) || body.textContent || ""));
    }
    ...
    function collectDeepSeekConversation() {
        ...
        items.forEach((item) => {
            ...
            if (role === "user") {
                const text = readExportText(body) || readExportText(host);
                if (!text && !assets.length) {
                    return;
                }
                userMessageIndex += 1;
                messages.push({
                    ...
                    sequenceIndex: getDeepSeekMessageSortKey({ turnId }),
                    sequenceSource: "dom",
                    index: userMessageIndex,
                    role: "user",
                    text,
    ...
    function findTurnElement(turnId, message) {
        if (message && message.platform === "deepseek") {
            ...
            if (turnId) {
                ...
                const deepSeekTurn = querySelectorSafe(buildAttributeSelector(DEEPSEEK_TURN_ID_ATTR, turnId));
                if (deepSeekTurn instanceof HTMLElement) {
                    return deepSeekTurn;
                }
            }
            return null;
    ...
    function findCapturedAssetUrl(sinceTs, filename, fileIdHints) {
        ...
        const pageBridge = window.ChatGPTExporterPageBridge;
        const pageNetworkEvents = pageBridge && typeof pageBridge.getEvents === "function" ? pageBridge.getEvents() : [];
