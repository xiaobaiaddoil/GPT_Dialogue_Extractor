...
一个用于导出 ChatGPT 和 DeepSeek 网页版当前会话的浏览器扩展。
直接读取页面中已经渲染完成的对话内容，并导出为本地文件。
...
进入任意一个 ChatGPT 或 DeepSeek 会话页面：
...
适合完整归档当前会话；ZIP 内包含 `conversation.json`、`conversation.md`、打包清单，以及可直接读取到的图片和附件。
...
扩展只读取当前页面 DOM 中已经渲染出来的消息。

如果很长的旧消息还没有加载到页面里，请先在网页端滚动到对应位置，让页面把这些消息渲染出来，再点击导出或刷新列表。
...
1. 先运行 `npm run build`
2. 在扩展管理页刷新扩展
3. 回到 ChatGPT 或 DeepSeek 页面刷新标签页
...
如果你的目标是稳定地把当前 ChatGPT 或 DeepSeek 网页版会话导出为本地文件，这个版本已经可以直接使用。
